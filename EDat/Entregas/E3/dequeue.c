...
void *dequeue_popBack(Dequeue *q){
  ...
  /* error checks */
  if (!q || dequeue_isEmpty(q)) return NULL;

  last_elem = (MAX_DEQUEUE + q->rear - 1) % MAX_DEQUEUE;

  e = q->data[last_elem];
  q->data[last_elem] = NULL;
  q->rear = (MAX_DEQUEUE + q->rear - 1) % MAX_DEQUEUE;
...
Status dequeue_pushFront(Dequeue *q, const void *ele){
  /* error checks */
  if (!q || !ele|| _dequeue_isFull(q)) return ERROR;

  if (dequeue_isEmpty(q))
    q->front = (MAX_DEQUEUE + q->front) % MAX_DEQUEUE;

  q->front = (MAX_DEQUEUE + q->front - 1) % MAX_DEQUEUE;
  q->data[q->front] = (void*)ele;
