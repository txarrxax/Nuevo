...
Status sl_append(SList *l, char *str) {
  ...
  /* check if the SList is full */
  if (_sl_isfull(l) == TRUE)
    _sl_grow(l);

  /* add the new string */
  l->data[sl_size(l)] = str;

  /* increase the number of elements */
  l->num_elem = sl_size(l) + 1;
...
Status sl_removeLast(SList *l, char **str) {
  ...
  /* save the last element in the string*/
  *str = l->data[last_position];
  
  /* remove the last element */
  l->data[last_position] = "";
  l->num_elem = last_position;
