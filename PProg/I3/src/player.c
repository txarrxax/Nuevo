...
Player *player_create() {
  ...
  p->location = NO_ID;
  (p->name)[0] = '\0';
  if ((p->inventory = inventory_create()) == NULL)
    return NULL;
  p->health = 0;
...
STATUS player_add_item(Player *p, const Id item_id) {
  ...
  if (inventory_add_object(p->inventory, item_id) == ERROR)
    return ERROR;
...
STATUS player_del_item(Player *p, const Id item_id) {
  ...
  if (inventory_del_object(p->inventory, item_id) == ERROR)
    return ERROR;
