...
STATUS set_add(Set *set, const Id id) {
  /*Error control*/
  if (!set || set->n_ids < 0 || id == NO_ID || !(set->ids))
    return ERROR;

  /*If the set is full we attempt to increase it*/
  if (set_is_full(set)) {
    if (set_grow(set) == ERROR)
      return ERROR;
  }
...
STATUS set_grow(Set *set) {
  ...
  if ((new_capacity = set->capacity * GROWTH_FACTOR) > SET_SIZE)
    return ERROR;

  /*Error control*/
  if (!(backup = realloc(set->ids, new_capacity * sizeof(Id))))
    return ERROR;
