...
int main(int argc, char **argv) {
  ...
  if (all || test == 9)
    test1_player_add_item();
  if (all || test == 10)
    test2_player_add_item();
  if (all || test == 11)
    test1_player_del_item();
  if (all || test == 12)
    test2_player_del_item();
  if (all || test == 13)
  ...
  if (all || test == 26)
    test2_player_print();
  if (all || test == 27)
    test1_player_set_inv_cap();
  if (all || test == 28)
    test2_player_set_inv_cap();
...
void test1_player_get_id() { PRINT_TEST_RESULT(player_get_id(NULL) == NO_ID); }
...
void test1_player_print() { PRINT_TEST_RESULT(player_print(NULL) == ERROR); }
