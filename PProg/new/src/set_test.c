...
int main(int argc, char **argv) {
  ...
  if (all || test == 13) test1_set_get_size();
  if (all || test == 14) test2_set_get_size();
  if (all || test == 15) test1_set_get_ids();
  if (all || test == 16) test2_set_get_ids();
  if (all || test == 17) test1_set_has_id();
  if (all || test == 18) test2_set_has_id();
...
void test1_set_has_id() {
  PRINT_TEST_RESULT(set_has_id(NULL, TEST_ID) == FALSE);
}
...
void test2_set_has_id() {
  ...
  PRINT_TEST_RESULT(set_has_id(s, NO_ID) == FALSE);
  set_destroy(s);
}
