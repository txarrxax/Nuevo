...
int main(int argc, char **argv) {
  ...
  test1_enemy_print();
  test2_enemy_print();
  test1_enemy_get_strength();
  test2_enemy_get_strength();
  test1_enemy_set_strength();
  test2_enemy_set_strength();
...
void test1_enemy_get_strength() {
  PRINT_TEST_RESULT(enemy_get_strength(NULL) == 0);
}

void test2_enemy_get_strength() {
  Enemy *e = enemy_create();
  if (!e) {
    PRINT_TEST_RESULT(FALSE);
    return;
  }

  PRINT_TEST_RESULT(enemy_get_strength(e) == 0);
  enemy_destroy(e);
}

void test1_enemy_set_strength() {
  PRINT_TEST_RESULT(enemy_set_strength(NULL, TEST_STRENGTH) == ERROR);
}
...
void test2_enemy_set_strength() {
  ...
  if (enemy_set_strength(e, TEST_STRENGTH) == ERROR) {
    PRINT_TEST_RESULT(FALSE);
    return;
  }

  PRINT_TEST_RESULT(enemy_get_strength(e) == TEST_STRENGTH);
  enemy_destroy(e);
}
